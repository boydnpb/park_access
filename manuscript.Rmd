...
tract_centroids <- gCentroid(denver_tracts, byid = TRUE) #if byid = FALSE, then centroid of "all" points
park_centroids <- gCentroid(denver_parks, byid = TRUE)
...
```{r denver_W}
neighbors <- poly2nb(denver_tracts, queen = FALSE)
...
denver_physact_sem2 <- errorsarlm(update(denver_parks_formula, PhysAct ~ .), 
                           data = denver_tracts@data, listw = W, zero.policy = TRUE)
